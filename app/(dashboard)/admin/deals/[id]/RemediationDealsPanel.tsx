...
export default function RemediationDealsPanel({
  ...
  const handleAdd = async () => {
    ...
    if (!form.propertyAddress.trim()) { setAddError('Property address is required'); return }
    if (!form.brokerageLegalName.trim()) { setAddError('Brokerage legal name is required'); return }
    if (!Number.isFinite(directed) || directed <= 0) { setAddError('Directed amount must be greater than zero'); return }
    ...
    if (result.success) {
      ...
      setAddError(result.error || 'Failed to create remediation deal')
    }
  }

...
3. **Notify the brokerage.** The brokerage admin team is emailed to submit the advance on the agent's behalf. The email (`renderBrokerageOfferEmail` in `render-brokerage-offer-email.ts`, sent via `dispatch-brokerage-offer.ts`) is **document and action focused with no dollar amounts or commission figures**: the brokerage tells us the real commission when they submit, so quoting our rough pre-split estimate would only mislead. Its job is to get the brokerage into the pre-filled portal form and **upload the deal documents** so we can fund. The subject for an accepted offer is "`<Agent>` requested an advance on `<address>`. Upload the documents to fund today" (a Tier A speculative lead instead leads with "Possible commission advance for `<Agent>`...", and the 2-hour nudge variant leads with "Reminder: ... is waiting on your documents"). This is best-effort: if Resend fails, the send is enqueued in `cron_email_failures` for the retry sweeper, and the agent is told "we'll keep trying." The offered deal is never rolled back because the notification failed. There is a recipient gate: if the brokerage has no email, no configured pipe recipients, and no `FIRM_FUNDS_OFFER_INBOX`, acceptance is refused with a clear support message rather than creating a black hole.

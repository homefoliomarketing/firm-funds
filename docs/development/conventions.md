...
The package carries an `audience: 'internal' | 'brokerage'` flag. `'brokerage'` (used by `/api/brokerage/reports/export`, scoped to the caller's own brokerage) strips every Firm Funds margin figure — the fee charged to the agent, total fee revenue, and gross profit — in BOTH the data (the builder zeroes them) and the rendered output (the generators drop those boxes/columns). When adding a field, decide whether it is brokerage-visible; if it is Firm Funds margin, zero it for brokerage audience in the builder AND hide it in both generators. `lib/reports/audience.test.ts` is a regression guard that fails if any margin value leaks into a brokerage workbook.

...
This document explains how Firm Funds sends contracts for e-signature through SignWell, how field placement works with hidden text tags, how the webhook processes completions, where signed documents are stored, the runtime provider flag that toggles between SignWell and DocuSign, and the open items that still need live validation.
...
- **Regular deal (CPA + IDP)**: for each envelope row, store the PDF under `{dealId}/...`, insert a `deal_documents` record (`commission_agreement` or `direction_to_pay`, `upload_source = 'nexone_auto'`, note "Signed via SignWell"), then find the matching `underwriting_checklist` item and auto-check it, linking the document.
...
- **CC recipients dropped.** SignWell treats every recipient as a signer, so the DocuSign carbon-copy recipient (the Broker of Record / brokerage admin) is **not** included on the SignWell path. `TODO`: notify them via Resend instead.

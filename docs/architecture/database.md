...
| 100 to 118 | Underwriter assigned_at, KYC bucket limits, staff least-privilege roles, look-only impersonation, brokerage OG image, agent self-submit, informational statement ledger, signed-BCA path / welcome flag / deposit-auth consent, human-readable deal numbers, SignWell webhook events, optional brokerage flat fee, multi-document checklist linking, per-deal agent-refund tracking + completion gate, brokerage-payments junction RLS, per-brokerage firm-deal notification channels, firm-deal commission hold, firm-deal agent pre-request, funded-amendment brokerage recalc, deal-completion receipt document type |

There is no `095_firm_deal_offer_expiry.sql` in the repository. The actual `095` file is `095_fix_brokerage_admins_recursion.sql`. The highest-numbered migration on disk is `118_completion_receipt_document_type.sql`.
...
| agent_pre_request_at / second_agent_pre_request_at | TIMESTAMPTZ | Per-side stamp that the matched agent "pre-requested" the advance from the onboarding "You're all set" page before account activation. The activation hook fires acceptance for the side once `account_activated_at` flips; ignored once that side's `offer_deal_id` / `second_offer_deal_id` is set (migration 116) |
| commission_hold_since | TIMESTAMPTZ | Set when the event was parked one poll cycle to wait for the brokerage to enter the commission; NULL once released (migration 115) |
...
Base table (predates migration tracking). The `document_type` CHECK constraint is widened across migrations 041 (closing_date_amendment), 042 (banking_info), and 118 (completion_invoice). The user-facing manual-upload type list lives in `lib/constants.ts` (`DOCUMENT_TYPES`): aps, amendment, trade_record, mls_listing, commission_agreement, direction_to_pay, notice_of_fulfillment, kyc_fintrac, id_verification, brokerage_cooperation_agreement, closing_date_amendment, banking_info, other. `completion_invoice` is in the DB CHECK but deliberately NOT in `DOCUMENT_TYPES`: it is a system-generated deal-completion receipt inserted directly via the service-role client (never a manual choice), and it is agent-private (filtered out of the brokerage portal via `BROKERAGE_HIDDEN_DOC_TYPES` because it shows the agent's service fee, which is Firm Funds' margin). Upload source is `nexone_auto`, `manual_upload`, or `system` (the last for app-generated docs like the completion receipt; migration 118). Related: `document_returns` (migration 018) for returning incorrect docs to agents.
...
| 118_completion_receipt_document_type.sql | Adds `completion_invoice` to the `deal_documents` document_type CHECK and `system` to the upload_source CHECK. Backs the new deal-completion receipt: when a deal completes, the app auto-generates a one-page receipt PDF, stores it as a `completion_invoice` deal_documents row (`upload_source = 'system'`), and emails it to the agent. The receipt is agent-private (hidden from the brokerage portal via `BROKERAGE_HIDDEN_DOC_TYPES`). Additive; no money math touched |

...
| `/api/cron/firm-deal-poller` | Read brokerage Google Sheets and insert detected events, release any commission holds due, then parse+match new events and dispatch approved ones — the whole pipeline in one job. | Every 15 minutes | 15-minute bucket |
...
Loads every enabled `brokerage_pipes` row of `pipe_type='spreadsheet'`, reads the configured tabs from each brokerage's Google Sheet (read-only), diffs against `last_poll_state`, and inserts a `firm_deal_events` row (`status='new'`) for each detected trigger. As part of each pipe's poll it also releases any `commission_hold` events parked on a prior cycle, re-reading the now-fresh row for a commission that may have been entered (Stage 3.5 in `business/firm-deals.md`). It then runs the rest of the pipeline inline: a `releaseStaleCommissionHolds` safety sweep, `processAllNewEvents` (parse + agent-match every `new` event, which may newly park a commission hold), then `dispatchApprovedEvents` (send any `approved` events). Because poll+release (Stage 1) runs before processing (Stage 2), a hold created this run waits until the next run — exactly one cycle. Polling persists `last_poll_state` first, so a slow parse can't lose a detection — leftovers retry next run. This single 15-minute job replaced the former every-2-minute processor + dispatcher crons, which kept tripping request timeouts and being auto-disabled by cron-job.org. Source: `app/api/cron/firm-deal-poller/route.ts`.
